...
const VerifyEmail = () => {
  ...
  return (
    <div className="min-h-[90vh] flex items-center justify-center bg-[#EDF7BD] pt-32 pb-16 px-4">
      <div className="max-w-md w-full py-12 px-10 bg-[#EDF7BD] shadow-2xl rounded-xl text-center space-y-8 transition-all duration-700">
        {status === 'verifying' && (
        ...
        {status === 'success' && (
          <div className="animate-in zoom-in duration-500 space-y-6">
            ...
            <Link to="/login" className="inline-block w-full bg-[#EDF7BD] text-black py-4 rounded-xl font-bold uppercase tracking-widest hover:bg-[#EDF7BD] transition-all shadow-lg mt-4">
              Step Inside
            </Link>
          </div>
        )}
