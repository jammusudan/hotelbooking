...
const MyBookings = () => {
    ...
    return (
        <div className="bg-[#85C79A] min-h-screen pt-32 pb-24 relative overflow-hidden">
                ...
                <div className="mb-20 flex flex-col md:flex-row justify-between items-end gap-8 border-b border-white/5 pb-10">
                    <div>
                        ...
                        <h2 className="text-5xl md:text-7xl font-serif font-black text-white uppercase tracking-tighter leading-none italic">
                            My <span className="text-white">Bookings</span>
                        </h2>
                        <p className="text-white mt-4 font-medium tracking-wide italic">History of your stays with Navan.</p>
                    </div>
                </div>

                {bookings.length === 0 ? (
                    <div className="text-center py-32 bg-[#85C79A] rounded-[3rem] border border-dashed border-white/10 border-2">
                    ...
                    <div className="grid grid-cols-1 gap-12">
                        {bookings.map((booking) => (
                            <div key={booking._id} className="group bg-[#85C79A] rounded-[3rem] border border-white/5 shadow-2xl hover:border-[#0B2D72]/30 transition-all duration-700 overflow-hidden flex flex-col lg:flex-row shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)]">
                                ...
                                <div className="lg:w-1/3 min-h-[300px] overflow-hidden relative">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-r from-[#85C79A] via-transparent to-transparent opacity-60"></div>
                                    <div className="absolute top-8 left-8">
                                        <span className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl backdrop-blur-md border border-white/10 ${
                                            ...
                                            booking.status === 'Cancelled' ? 'bg-red-500/20 text-red-400' : 'bg-[#0B2D72]/20 text-white'
                                        }`}>
                                            {booking.status}
                                        </span>
                                    </div>
                                ...
                                <div className="flex-grow p-12 flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start mb-8">
                                            <div>
                                                <h3 className="text-3xl font-serif font-black text-white hover:text-white transition-colors uppercase tracking-tight italic mb-2">
                                                ...
                                                <p className="text-[10px] font-black text-white uppercase tracking-[0.3em] flex items-center gap-2">
                                                    <MapPin size={10} className="text-white" /> {booking.hotelId?.city}, {booking.hotelId?.country}
                                                </p>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-3xl font-serif font-black text-white leading-none">₹{booking.totalAmount}</div>
                                                <div className="text-[9px] font-black text-white uppercase mt-2 tracking-widest">Total Amount</div>
                                            </div>
                                        </div>

                                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 p-10 bg-[#85C79A]/40 rounded-[2.5rem] border border-white/5">
                                    ...
                                    <div className="mt-12 flex flex-wrap gap-4">
                                        {booking.status === 'Pending' && (
                                            <Link to={`/payment/${booking._id}`} className="bg-[#0B2D72] text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-[#0B2D72]/10 flex items-center gap-2">
                                        ...
                                        {booking.status === 'Confirmed' && (
                                            <button 
                                                ...
                                                className="bg-[#0B2D72] text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-[#0B2D72]/10 flex items-center gap-2"
                                            >
                                        ...
                                        {booking.paymentStatus === 'Paid' && booking.status === 'Cancelled' && (
                                            <button 
                                                ...
                                                className="bg-[#0B2D72]/10 text-white border border-[#0B2D72]/20 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#0B2D72] hover:text-white transition-all"
                                            >
                                                Request Refund
                                            </button>
                                        )}
                                        {booking.paymentStatus === 'Refunded' && (
                                            <span className="bg-white/5 text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/5 flex items-center gap-2">
                                        ...
                                        {(booking.status === 'Confirmed' || booking.status === 'Pending') && new Date(booking.checkIn) > new Date() && (
                                            <>
                                                <button 
                                                    ...
                                                    className="bg-white/5 text-white border border-white/10 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#0B2D72] hover:text-white transition-all flex items-center gap-2"
                                                >
            ...
            {/* MODIFICATION MODAL */}
            {editingBooking && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#85C79A]/80 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-[#85C79A] w-full max-w-xl rounded-[3rem] shadow-2xl relative border border-white/10 overflow-hidden animate-in zoom-in-95 duration-500">
                        ...
                        <div className="p-16">
                            <div className="text-center mb-12">
                                <h3 className="text-[10px] font-black text-white uppercase tracking-[0.5em] mb-4">Booking Details</h3>
                            ...
                            <form onSubmit={handleUpdate} className="space-y-8">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-white uppercase tracking-widest pl-2">Revised Arrival</label>
                                        <input 
                                            type="date" required 
                                            value={editForm.checkIn}
                                            onChange={(e) => setEditForm({...editForm, checkIn: e.target.value})}
                                            className="w-full bg-[#85C79A]/40 border border-white/10 rounded-2xl p-4 text-sm text-white focus:border-[#0B2D72] outline-none transition-all font-bold color-scheme-dark"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-white uppercase tracking-widest pl-2">Revised Departure</label>
                                        <input 
                                            type="date" required 
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-white uppercase tracking-widest pl-2">Accompanying Guests</label>
                                    <input 
                                        type="number" min="1" required 
                                        value={editForm.guests}
                                        onChange={(e) => setEditForm({...editForm, guests: parseInt(e.target.value)})}
                                        className="w-full bg-[#85C79A]/40 border border-white/10 rounded-2xl p-4 text-sm text-white focus:border-[#0B2D72] outline-none transition-all font-bold"
                                    />
                                </div>
            ...
            {/* REVIEW MODAL */}
            {reviewingBooking && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-[#85C79A]/80 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-[#85C79A] w-full max-w-xl rounded-[3rem] shadow-2xl relative border border-white/10 overflow-hidden animate-in zoom-in-95 duration-500">
                        <div className="absolute top-0 left-0 w-full h-2 bg-[#0B2D72]"></div>
                        ...
                        <div className="p-16">
                            ...
                            <form onSubmit={submitReview} className="space-y-10">
                                <div className="space-y-4">
                                    ...
                                    <div className="flex justify-center gap-6 py-8 bg-[#85C79A]/40 rounded-[2.5rem] border border-white/5">
                                        {[1, 2, 3, 4, 5].map((star) => (
                                            <button 
                                ...
                                <div className="space-y-3">
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full bg-[#85C79A]/40 border border-white/10 rounded-[2rem] p-8 text-white focus:border-[#0B2D72] outline-none text-sm font-medium transition-all resize-none placeholder:text-gray-700 italic"
                                    />
                                </div>
