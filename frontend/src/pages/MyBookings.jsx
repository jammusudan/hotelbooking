...
const MyBookings = () => {
    ...
    return (
        <div className="bg-[#0a0a0b] min-h-screen pt-32 pb-24 relative overflow-hidden">
            ...
            <div className="absolute top-0 right-0 w-96 h-96 bg-gold-900/5 blur-[120px] pointer-events-none"></div>
            <div className="absolute bottom-0 left-0 w-96 h-96 bg-gold-900/5 blur-[120px] pointer-events-none"></div>
                ...
                <div className="mb-20 flex flex-col md:flex-row justify-between items-end gap-8 border-b border-white/5 pb-10">
                    <div>
                        ...
                        <h2 className="text-5xl md:text-7xl font-serif font-black text-white uppercase tracking-tighter leading-none italic">
                            My <span className="text-gold-500">Bookings</span>
                        </h2>
                        <p className="text-gray-500 mt-4 font-medium tracking-wide italic">History of your stays with Navan.</p>
                    </div>
                </div>

                {bookings.length === 0 ? (
                    <div className="text-center py-32 bg-[#111113] rounded-[3rem] border border-dashed border-white/10 border-2">
                        ...
                        <Link to="/hotels" className="inline-block bg-gold-500 text-black px-12 py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-white transition-all transform active:scale-95 shadow-2xl shadow-gold-500/10">Find Hotels</Link>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-12">
                        {bookings.map((booking) => (
                            <div key={booking._id} className="group bg-[#111113] rounded-[3rem] border border-white/5 shadow-2xl hover:border-gold-500/30 transition-all duration-700 overflow-hidden flex flex-col lg:flex-row shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)]">
                                ...
                                <div className="lg:w-1/3 min-h-[300px] overflow-hidden relative">
                                    ...
                                    <div className="absolute inset-0 bg-gradient-to-r from-[#111113] via-transparent to-transparent opacity-60"></div>
                                    <div className="absolute top-8 left-8">
                                        <span className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl backdrop-blur-md border border-white/10 ${
                                            ...
                                            booking.status === 'Cancelled' ? 'bg-red-500/20 text-red-400' : 'bg-gold-500/20 text-gold-400'
                                        }`}>
                                            {booking.status}
                                        </span>
                                    </div>
                                ...
                                <div className="flex-grow p-12 flex flex-col justify-between">
                                    <div>
                                        <div className="flex justify-between items-start mb-8">
                                            <div>
                                                <h3 className="text-3xl font-serif font-black text-white hover:text-gold-500 transition-colors uppercase tracking-tight italic mb-2">
                                                ...
                                                <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] flex items-center gap-2">
                                                    <MapPin size={10} className="text-gold-500" /> {booking.hotelId?.city}, {booking.hotelId?.country}
                                                </p>
                                            </div>
                                            <div className="text-right">
                                                <div className="text-3xl font-serif font-black text-gold-500 leading-none">₹{booking.totalAmount}</div>
                                                <div className="text-[9px] font-black text-gray-500 uppercase mt-2 tracking-widest">Total Amount</div>
                                            </div>
                                        </div>

                                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 p-10 bg-black/40 rounded-[2.5rem] border border-white/5">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <strong className="text-xs font-black text-gray-300 uppercase tracking-widest">{booking.roomId?.type}</strong>
                                            </div>
                                            <div className="space-y-2">
                                    ...
                                    <div className="mt-12 flex flex-wrap gap-4">
                                        {booking.status === 'Pending' && (
                                            <Link to={`/payment/${booking._id}`} className="bg-gold-500 text-black px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-gold-500/10 flex items-center gap-2">
                                        ...
                                        {booking.status === 'Confirmed' && (
                                            <Link 
                                                ...
                                                className="bg-white/5 text-white border border-white/10 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-black transition-all flex items-center gap-2"
                                            >
                                        ...
                                        {booking.paymentStatus === 'Paid' && booking.status === 'Cancelled' && (
                                            <button 
                                                ...
                                                className="bg-gold-500/10 text-gold-500 border border-gold-500/20 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gold-500 hover:text-white transition-all"
                                            >
                                                Request Refund
                                            </button>
                                        )}
                                        {booking.paymentStatus === 'Refunded' && (
                                            <span className="bg-white/5 text-gray-500 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/5 flex items-center gap-2">
                                        ...
                                        {(booking.status === 'Confirmed' || booking.status === 'Pending') && new Date(booking.checkIn) > new Date() && (
                                            <>
                                                <button 
                                                    ...
                                                    className="bg-white/5 text-white border border-white/10 px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-gold-500 hover:text-white transition-all flex items-center gap-2"
                                                >
                                                    <Edit3 size={14} /> Edit Dates
                                                </button>
                                                <button 
                                                    onClick={() => handleCancel(booking._id)} 
                                                    className="bg-rose-500/10 text-rose-500 border border-rose-500/20 hover:bg-rose-500 hover:text-white px-10 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                                                >
            ...
            {editingBooking && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-[#111113] w-full max-w-xl rounded-[3rem] shadow-2xl relative border border-white/10 overflow-hidden animate-in zoom-in-95 duration-500">
                        ...
                        <button onClick={() => setEditingBooking(null)} className="absolute top-10 right-10 text-gray-500 hover:text-white transition-colors outline-none z-10">
                            <X size={24} />
                        </button>
                        
                        <div className="p-16">
                            <div className="text-center mb-12">
                                <h3 className="text-[10px] font-black text-gold-500 uppercase tracking-[0.5em] mb-4">Booking Details</h3>
                                <h4 className="text-4xl font-serif font-black text-white uppercase tracking-tight italic leading-none">Edit Booking</h4>
                            </div>
                            
                            <form onSubmit={handleUpdate} className="space-y-8">
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-2">Revised Arrival</label>
                                        <input 
                                            type="date" required 
                                            value={editForm.checkIn}
                                            onChange={(e) => setEditForm({...editForm, checkIn: e.target.value})}
                                            className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-sm text-white focus:border-gold-500 outline-none transition-all font-bold color-scheme-dark"
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-2">Revised Departure</label>
                                        <input 
                                            type="date" required 
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-2">Accompanying Guests</label>
                                    <input 
                                        type="number" min="1" required 
                                        value={editForm.guests}
                                        onChange={(e) => setEditForm({...editForm, guests: parseInt(e.target.value)})}
                                        className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-sm text-white focus:border-gold-500 outline-none transition-all font-bold"
                                    />
                                </div>
            ...
            {reviewingBooking && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-500">
                    <div className="bg-[#111113] w-full max-w-xl rounded-[3rem] shadow-2xl relative border border-white/10 overflow-hidden animate-in zoom-in-95 duration-500">
                        <div className="absolute top-0 left-0 w-full h-2 bg-gold-500"></div>
                        <button onClick={() => setReviewingBooking(null)} className="absolute top-10 right-10 text-gray-500 hover:text-white transition-colors outline-none z-10">
                            <X size={24} />
                        </button>
