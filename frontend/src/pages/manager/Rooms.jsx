    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-8 mb-12">
        ...
        <button 
          ...
          className="flex items-center justify-center gap-3 bg-transparent text-white px-8 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-[#003049] hover:text-white transition-all shadow-lg shadow-transparent/20 active:scale-95 group disabled:opacity-30"
        >
      ...
      {loading && rooms.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          {rooms.map((room) => (
            <div key={room._id} className="bg-transparent rounded-[2.5rem] p-8 border border-white/10/50 hover:border-transparent/30 transition-all group relative flex flex-col shadow-2xl">
              <div className="flex justify-between items-start mb-8 pb-6 border-b border-white/10/50">
                <div className="p-4 bg-transparent/10 text-white rounded-2xl border border-transparent/20 shadow-inner group-hover:scale-110 transition-transform">
                  <Bed className="w-7 h-7" />
                </div>
                <div className="flex gap-4">
                  <button 
                    onClick={() => handleOpenModal(room)}
                    className="p-3 bg-[#003049] text-white hover:text-white hover:bg-gray-800 rounded-xl transition-all border border-white/10"
                  >
              ...
              <div className="grid grid-cols-2 gap-8 mb-10 pb-8 border-b border-white/10/50">
              ...
              <div className="flex flex-wrap gap-2.5 mt-auto">
                {room.amenities.map(a => (
                  <span key={a} className="bg-[#003049]/50 text-white px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border border-white/10 group-hover:border-transparent/20 transition-colors">
                    {a}
                  </span>
                ))}
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3">
                  ...
                  <select 
                    required 
                    className="w-full px-6 py-4 bg-[#003049]/50 border border-white/10 rounded-2xl focus:border-transparent/50 outline-none font-bold text-white transition-all shadow-inner appearance-none cursor-pointer"
              ...
              <div className="flex items-center gap-4 bg-[#003049]/30 p-6 rounded-2xl border border-white/10 transition-colors hover:border-red-500/30 group">
                <input 
                  type="checkbox" 
                  id="maintenance" 
                  className="w-6 h-6 border-white/10 bg-[#003049] rounded-lg checked:bg-transparent checked:border-transparent appearance-none cursor-pointer transition-all flex items-center justify-center after:content-[''] after:hidden checked:after:block after:w-2 after:h-4 after:border-r-2 after:border-b-2 after:border-black after:rotate-45"
