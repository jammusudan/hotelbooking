            ...
            <div key={review._id} className="bg-[#111114] rounded-[2.5rem] p-10 border border-gray-800/50 hover:border-[#0B2D72]/30 transition-all group relative flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="mb-10 flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#7FB77E] border border-gray-800 flex items-center justify-center text-black font-black shadow-inner group-hover:scale-110 transition-transform">
                    {review.userId?.name?.charAt(0)}
                  </div>
                  <div>
                    <h3 className="font-serif font-black text-black text-lg italic tracking-tight uppercase">{review.userId?.name}</h3>
                    <p className="text-[9px] font-black text-black/60 uppercase tracking-widest mt-1 italic">Verified Guest</p>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2">
                  ...
                  <span className="text-[8px] font-black text-black uppercase tracking-widest italic">{new Date(review.createdAt).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              <div className="mb-10 flex-1 relative">
                ...
                <blockquote className="text-black text-sm leading-relaxed italic font-medium group-hover:text-black transition-colors">
                  "{review.comment}"
                </blockquote>
                ...
                {review.managerResponse && (
                  <div className="mt-8 bg-[#0B2D72]/5 p-6 rounded-2xl border border-[#0B2D72]/10 animate-in slide-in-from-top-4">
                    ...
                    <p className="text-xs text-black italic font-medium leading-relaxed">{review.managerResponse}</p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-8 border-t border-gray-800/50">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2 overflow-hidden">
                    <Hotel className="w-4 h-4 text-black/40 flex-shrink-0" />
                    <p className="text-[9px] font-black text-black uppercase tracking-widest group-hover:text-black/60 transition-colors truncate">{review.hotelId?.name}</p>
                  </div>
                ...
                {activeReplyId === review._id && (
                  <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-black text-black uppercase tracking-widest italic">Drafting Protocol</span>
                      <button onClick={() => setActiveReplyId(null)} className="text-black hover:text-black transition-colors">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                    <textarea 
                      className="w-full p-6 bg-[#7FB77E]/50 border border-gray-800 rounded-2xl focus:border-[#0B2D72]/50 outline-none text-sm font-bold text-black transition-all shadow-inner placeholder:text-black resize-none italic"
                    ...
                    <div className="flex justify-end">
                      <button 
                        ...
                        className="flex items-center gap-3 bg-[#0B2D72] text-black px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-[#7FB77E] transition-all disabled:opacity-30 shadow-lg shadow-[#0B2D72]/20"
                      >
