    ...
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700 font-sans text-white">
      ...
      {loading && reviews.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10">
          {reviews.map((review) => (
            <div key={review._id} className="bg-transparent rounded-[2.5rem] p-10 border border-white/10/50 hover:border-transparent/30 transition-all group relative flex flex-col shadow-2xl overflow-hidden">
              ...
              <div className="mb-10 flex justify-between items-start">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-[#003049] border border-white/10 flex items-center justify-center text-white font-black shadow-inner group-hover:scale-110 transition-transform">
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i} 
                        className={`w-3 h-3 ${i < review.rating ? 'fill-[transparent] text-white' : 'text-white shadow-inner'}`} 
                      />
                    ))}
                  </div>
                  <span className="text-[8px] font-black text-white uppercase tracking-widest italic">{new Date(review.createdAt).toLocaleDateString()}</span>
                </div>
              </div>
              ...
              <div className="pt-8 border-t border-white/10/50">
                <div className="flex items-center justify-between mb-6">
                  ...
                  {!review.managerResponse && activeReplyId !== review._id && (
                    <button 
                      ...
                      className="text-[9px] font-black text-white hover:text-white uppercase tracking-widest transition-colors flex items-center gap-2 bg-transparent/10 px-4 py-2 rounded-xl border border-transparent/10 hover:bg-transparent hover:text-white"
                    >
                ...
                {activeReplyId === review._id && (
                  <div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                    ...
                    <textarea 
                      className="w-full p-6 bg-[#003049]/50 border border-white/10 rounded-2xl focus:border-transparent/50 outline-none text-sm font-bold text-white transition-all shadow-inner placeholder:text-white resize-none italic"
                    ...
                    <div className="flex justify-end">
                      <button 
                        ...
                        className="flex items-center gap-3 bg-transparent text-white px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-[#003049] transition-all disabled:opacity-30 shadow-lg shadow-transparent/20"
                      >
      ...
      {reviews.length === 0 && !loading && (
        <div className="text-center py-32 bg-transparent rounded-[3rem] border border-white/10/50 border-dashed group">
          <MessageSquare className="w-16 h-16 text-white mx-auto mb-8 group-hover:text-white/20 transition-colors animate-pulse" />
          <p className="text-white font-bold uppercase tracking-[0.3em] text-[10px] italic">Telemetry null: No guest testimonials retrieved.</p>
        </div>
      )}
    </div>
  );
