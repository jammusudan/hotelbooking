...
const Navbar = () => {
  ...
  const navClasses = `fixed w-full z-50 transition-all duration-300 border-b ${
     scrolled 
        ? 'bg-[#0a0a0b]/90 backdrop-blur-xl border-gray-800/50 py-3 shadow-2xl' 
        : (isHome ? 'bg-transparent border-transparent py-5' : 'bg-[#0a0a0b] border-gray-800 py-4 shadow-lg')
  }`;

  const linkColor = scrolled ? 'text-gray-300 hover:text-gold-500' : (isHome ? 'text-white hover:text-gold-400' : 'text-gray-300 hover:text-gold-500');
  const logoColor = 'text-gold-500 hover:text-gold-400';
  ...
  return (
    <nav className={navClasses}>
            ...
            {user ? (
              <div className="relative group flex items-center space-x-6 pl-6 border-l border-gray-300/30">
                <div className="flex flex-col items-end">
                  <span className={`text-[10px] uppercase font-black tracking-widest ${scrolled ? 'text-gray-500' : 'text-gray-400'}`}>{user.role}</span>
              ...
              <div className="space-x-4 flex items-center pl-6 border-l border-gray-300/30">
                ...
                <Link to="/register" className="bg-gold-500 hover:bg-gold-600 text-white text-sm font-bold px-6 py-2.5 rounded-full shadow-lg hover:shadow-xl transition-all">Join Now</Link>
              </div>
            )}
          </div>
      ...
      <div 
        className={`md:hidden absolute top-full left-0 w-full bg-[#0a0a0b]/95 backdrop-blur-3xl border-b border-gray-800 shadow-2xl transition-all duration-300 overflow-hidden ${
          isMobileMenuOpen ? 'max-h-[500px] py-4' : 'max-h-0 py-0 border-transparent'
        }`}
      >
        <div className="px-6 flex flex-col space-y-4">
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-300 hover:text-gold-500 font-medium py-2 border-b border-gray-800/50">Home</Link>
          <Link to="/hotels" onClick={() => setIsMobileMenuOpen(false)} className="text-gray-300 hover:text-gold-500 font-medium py-2 border-b border-gray-800/50">Destinations</Link>
          
          {user ? (
            <div className="pt-2 flex flex-col space-y-4">
              <div className="flex items-center gap-3 py-2">
                <div className="w-10 h-10 rounded-full bg-gold-500 flex items-center justify-center text-black font-black text-lg">
                  {user.name.charAt(0)}
                </div>
                <div>
                  <p className="text-white font-bold">{user.name}</p>
                  <p className="text-[10px] uppercase font-black tracking-widest text-gold-500">{user.role}</p>
                </div>
              </div>
            ...
            <div className="pt-4 flex flex-col space-y-3">
              ...
              <Link to="/register" onClick={() => setIsMobileMenuOpen(false)} className="text-center bg-gold-500 hover:bg-gold-600 text-white font-bold py-3 rounded-xl shadow-lg transition-all">Join Now</Link>
            </div>
          )}
        </div>
      </div>
